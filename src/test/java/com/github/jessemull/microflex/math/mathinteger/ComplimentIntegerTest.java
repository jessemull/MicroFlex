...
import com.github.jessemull.microflex.integerflex.math.ComplimentInteger;
...
public class ComplimentIntegerTest {

    /* ---------------------------- Local fields ---------------------------- */
	
    /* Minimum and maximum values for random well and lists */
	
	private static int minValue = -100000;          // Minimum big integer value for wells
	private static int maxValue = 1000000;          // Maximum big integer value for wells
	...
	private static ComplimentInteger compliment = new ComplimentInteger();
	...
	public void testConstructor() {
		ComplimentInteger test = new ComplimentInteger();
		assertNotNull(test);
	}
		...
		for(PlateInteger[] plates : arrays) {
			...
			for(PlateInteger plate : plates) {
				...
				for(WellInteger well : plate) {
					...
					List<Integer> returned = compliment.wells(well);
        ...
        for(PlateInteger[] plates : arrays) {
			...
			for(PlateInteger plate : plates) {
				...
				for(WellInteger well : plate) {
	    			...
	    			List<Integer> result = new ArrayList<Integer>();
					List<Integer> returned = compliment.wells(well, begin, end - begin);				
					
					for(int i = begin; i < end; i++) {
						result.add(~well.data().get(i));
					}
    ...
    public void testPlates() {
        ...
        for(PlateInteger[] plates : arrays) {
			...
			for(PlateInteger plate : plates) {
				...
				PlateInteger returnedPlate = compliment.plates(plate);
    	...
    	for(PlateInteger[] plates : arrays) {
			...
			for(PlateInteger plate : plates) {
				...
				for(WellInteger well : plate) {
					...
					for(int i = begin; i < end; i++) {
						resultList.add(~well.data().get(i));
					}
				...
				PlateInteger resultPlate = new PlateInteger(plate.rows(), plate.columns(), result);
				PlateInteger returnedPlate = compliment.plates(plate, begin, end - begin);
    ...
    public void testSets() {
        ...
        for(PlateInteger[] plates : arrays) {
			...
			for(PlateInteger plate : plates) {
				...
				WellSetInteger result = new WellSetInteger();
				WellSetInteger returned = compliment.sets(plate.dataSet());
    ...
    public void testSetsIndices() {
        ...
        for(PlateInteger[] plates : arrays) {
			...
			for(PlateInteger plate : plates) {
				...
				WellSetInteger result = new WellSetInteger();
				WellSetInteger returned = compliment.sets(plate.dataSet(), begin, end - begin);
    ...
    public void testStacks() {
        ...
        for(StackInteger stack : stacks) {
			...
			StackInteger returnedStack = compliment.stacks(stack);
    ...
    public void testStacksIndices() {
    	...
    	for(StackInteger stack : stacks) {
			...
			StackInteger returnedStack = compliment.stacks(stack, begin, end - begin);
