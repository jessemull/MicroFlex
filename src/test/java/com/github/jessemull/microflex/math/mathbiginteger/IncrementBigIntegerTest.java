...
import com.github.jessemull.microflex.bigintegerflex.math.IncrementBigInteger;
...
/**
 * This class tests the methods in the increment big integer class.
 * 
 * @author Jesse L. Mull
...
public class IncrementBigIntegerTest {
	...
	private static IncrementBigInteger increment = new IncrementBigInteger();
	...
	public void testConstructor() {
		IncrementBigInteger test = new IncrementBigInteger();
		assertNotNull(test);
	}
    ...
    public void testWells() {
		...
		for(PlateBigInteger plate : plates) {
			...
			for(WellBigInteger well : plate) {
				...
				List<BigInteger> returned = increment.wells(well);
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
	    			...
	    			List<BigInteger> result = new ArrayList<BigInteger>();
					List<BigInteger> returned = increment.wells(well, begin, end - begin);				
					
					for(int i = begin; i < end; i++) {
						result.add(well.data().get(i).add(BigInteger.ONE));
					}
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
					...
					for(BigInteger bd : well) {
						resultList.add(bd.add(BigInteger.ONE));
					}
				...
				PlateBigInteger returnedPlate = increment.plates(plate);
    ...
    public void testPlatesIndices() {
    	...
    	for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				PlateBigInteger resultPlate = new PlateBigInteger(plate.rows(), plate.columns(), result);
				PlateBigInteger returnedPlate = increment.plates(plate, begin, end - begin);
    ...
    public void testSets() {
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				WellSetBigInteger result = new WellSetBigInteger();
				WellSetBigInteger returned = increment.sets(plate.dataSet());
    ...
    public void testSetsIndices() {
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				WellSetBigInteger result = new WellSetBigInteger();
				WellSetBigInteger returned = increment.sets(plate.dataSet(), begin, end - begin);
    ...
    public void testStacks() {
        ...
        for(StackBigInteger stack : stacks) {
			...
			StackBigInteger returnedStack = increment.stacks(stack);
    ...
    public void testStacksIndices() {
    	...
    	for(StackBigInteger stack : stacks) {
			...
			StackBigInteger returnedStack = increment.stacks(stack, begin, end - begin);
