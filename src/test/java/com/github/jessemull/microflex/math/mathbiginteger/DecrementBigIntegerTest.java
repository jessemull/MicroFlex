...
import com.github.jessemull.microflex.bigintegerflex.math.DecrementBigInteger;
...
/**
 * This class tests the methods in the decrement big integer class.
 * 
 * @author Jesse L. Mull
...
public class DecrementBigIntegerTest {
	...
	private static DecrementBigInteger decrement = new DecrementBigInteger();
	...
	public void testConstructor() {
		DecrementBigInteger test = new DecrementBigInteger();
		assertNotNull(test);
	}
    ...
    public void testWells() {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
					...
					List<BigInteger> returned = decrement.wells(well);
					
					for(BigInteger bd : well) {
						result.add(bd.subtract(BigInteger.ONE));
					}
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
	    			...
	    			List<BigInteger> result = new ArrayList<BigInteger>();
					List<BigInteger> returned = decrement.wells(well, begin, end - begin);				
					
					for(int i = begin; i < end; i++) {
						result.add(well.data().get(i).subtract(BigInteger.ONE));
					}
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
					...
					for(BigInteger bd : well) {
						resultList.add(bd.subtract(BigInteger.ONE));
					}
				...
				PlateBigInteger returnedPlate = decrement.plates(plate);
    	...
    	for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
					...
					for(int i = begin; i < end; i++) {
						resultList.add(well.data().get(i).subtract(BigInteger.ONE));
					}
				...
				PlateBigInteger resultPlate = new PlateBigInteger(plate.rows(), plate.columns(), result);
				PlateBigInteger returnedPlate = decrement.plates(plate, begin, end - begin);
    ...
    public void testSets() {
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				WellSetBigInteger result = new WellSetBigInteger();
				WellSetBigInteger returned = decrement.sets(plate.dataSet());
    ...
    public void testSetsIndices() {
        ...
        for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				WellSetBigInteger result = new WellSetBigInteger();
				WellSetBigInteger returned = decrement.sets(plate.dataSet(), begin, end - begin);
    ...
    public void testStacks() {
        ...
        for(StackBigInteger stack : stacks) {
			...
			StackBigInteger returnedStack = decrement.stacks(stack);
    ...
    public void testStacksIndices() {
    	...
    	for(StackBigInteger stack : stacks) {
			...
			StackBigInteger returnedStack = decrement.stacks(stack, begin, end - begin);
