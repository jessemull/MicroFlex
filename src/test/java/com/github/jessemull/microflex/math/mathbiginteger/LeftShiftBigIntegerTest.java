...
import com.github.jessemull.microflex.bigintegerflex.math.LeftShiftBigInteger;
...
/**
 * This class tests the methods in the left shift big integer class.
 * 
 * @author Jesse L. Mull
...
public class LeftShiftBigIntegerTest {
	...
	private static LeftShiftBigInteger shift = new LeftShiftBigInteger();
	...
	public void testConstructor() {
		LeftShiftBigInteger test = new LeftShiftBigInteger();
		assertNotNull(test);
	}
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
					...
					for(int i = begin; i < end; i++) {
						result.add(well.data().get(i).shiftLeft(n));
					}
    	...
    	for(PlateBigInteger[] plates : arrays) {
			...
			for(PlateBigInteger plate : plates) {
				...
				for(WellBigInteger well : plate) {
					...
					for(int i = begin; i < end; i++) {
						resultList.add(well.data().get(i).shiftLeft(n));
					}
