...
/**
 * This class calculates the population variance of double plate stacks, plates, 
 ...
 */
public class PopulationVarianceDouble extends DescriptiveStatisticDoubleWeights {
    ...
    public double calculate(List<Double> list, double[] weights) {
    	...
    	for(int i = 0; i < list.size(); i++) {
    		weighted.add(list.get(i) * weights[i]);
    	}

    	double mean = this.mean(weighted);
        double sum = 0;
        
        for(double db : weighted) {
           sum += Math.pow(db - mean, 2);
        }
