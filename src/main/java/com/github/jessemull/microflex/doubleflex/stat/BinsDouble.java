...
/**
 * This class splits up double plate stacks, plates, wells and well sets into bins 
 ...
 */
public class BinsDouble extends QuantileStatisticListsDouble {
     ...
     * @param    List<Double>    thresholds
     * @return                   the result
     */
    public List<List<Double>> calculate(List<Double> list, List<Double> thresholds) {
    	...
    	for(int i = 0; i < list.size(); i++) {
    		...
    		if(list.get(i) < thresholds.get(index)) {
     ...
     * @return                   the result
     */
    public List<List<Double>> calculate(List<Double> list, int begin, int length, List<Double> thresholds) {
        return this.calculate(list.subList(begin, begin + length), thresholds);
    };
}