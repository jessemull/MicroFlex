 ...
 */
public class PlateInteger implements Iterable<WellInteger>, Comparable<PlateInteger> {            
    ...
    private WellSetInteger data;              // Well data
     ...
     */
    public PlateInteger(int type) {
        this.groups = new TreeSet<WellList>();
        this.data = new WellSetInteger();
     ...
     */
    public PlateInteger(int rows, int columns) {
        this.groups = new TreeSet<WellList>();
        this.data = new WellSetInteger();
     ...
     */
    public PlateInteger(int type, String label) {
        this(type);
        this.label = label;
    }
     ...
     * @param    String    plate label
     */
    public PlateInteger(int rows, int columns, String label) {
        this(rows, columns);
        this.label = label;
    }
     ...
     */
    public PlateInteger(int type, WellSetInteger set) {  	
        this(type);
     ...
     * @param    WellSetInteger    plate well sets
     */
    public PlateInteger(int rows, int columns, WellSetInteger set) {
        this(rows, columns);
        ValUtil.validateSet(this.rows, this.columns, set);
        this.addWells(set);
    }
     ...
     * @param    int               plate type flag
     * @param    String            plate label
     * @param    WellSetInteger    plate well set
     */
    public PlateInteger(int type, String label, WellSetInteger data) {
        this(type, data);
        this.label = label;
    }
     ...
     */
    public PlateInteger(int rows, int columns, String label, WellSetInteger data) {
     ...
     * @param    PlateInteger    plate to clone
     */
    public PlateInteger(PlateInteger plate) {
        ...
        WellSetInteger toAdd = plate.dataSet();
        this.addWells(toAdd);
     ...
     */
    public Set<WellSetInteger> allGroups() {

    	Set<WellSetInteger> groups = new TreeSet<WellSetInteger>();
    	...
    	for(WellList list : this.groups) {    		
    		...
    		for(WellIndex index : list) {
    			...
    			WellInteger well = this.getWells(new WellInteger(index.row(), index.column()));
    			
    			if(well == null) {
    				well = new WellInteger(index.row(), index.column());
    			}
     ...
     */
    public WellSetInteger getGroups(String label) {
     ...
     * @return               well set containing the group wells
     */
    public WellSetInteger getGroups(WellList list) {
     ...
     */
    public Set<WellSetInteger> getGroups(Collection<WellList> collection) {
    	
    	Set<WellSetInteger> set = new TreeSet<WellSetInteger>();
    	
    	for(WellList list : collection) {
    		
    		WellSetInteger group = this.getGroups(list);
     ...
     * @return                          wells for each group
     */
    public Set<WellSetInteger> getGroups(WellList[] array) {
     ...
     */
    public Set<WellSetInteger> getGroups(List<String> labels) {
     ...
     * @param    WellInteger well    input   well 
     ...
     * @param     WellInteger    the input well
     ...
     * @param    WellInteger    the first input well
     * @param    WellInteger    the second input well
     ...
     * @return    the data set
     */
    public WellSetInteger dataSet() {
        return new WellSetInteger(this.data);
    }
     ...
     */
    public WellInteger[] toArray() {
        return (WellInteger[]) this.data.toWellArray();
    }
     ...
     */
    public boolean addWells(WellInteger well) {
        
        try {
            
            ValUtil.validateWell(this.rows, this.columns, well);
            
            boolean add = this.data.add(new WellInteger(well.row(), well.column(), well.data()));
     ...
     *                             in the set
     */
    public boolean addWells(WellSetInteger set) {
     ...
     */
    public boolean addWells(Collection<WellInteger> collection) {
     ...
     *                            in the array
     */
    public boolean addWells(WellInteger[] array) {
     ...
     */
    public boolean removeWells(WellInteger well) {
     ...
     */
    public boolean removeWells(WellSetInteger set) {
     ...
     *            of all wells
     */
    public boolean removeWells(Collection<WellInteger> collection) {
     ...
     * @return                    true on successful removal of all wells
     */
    public boolean removeWells(WellInteger[] wells) {
    ...
    public boolean removeWells(String wellList, String delimiter) {
        ...
        for(String well : split) {
            try {
            	WellInteger toRemove = new WellInteger(well);
    ...
    public boolean removeWells(WellIndex index) {
        ...
        try {
        	...
        	WellInteger toRemove = new WellInteger(index.row(), index.column());
    ...
    public boolean removeWells(WellList list) {
        ...
        for(WellIndex index : list) {
            try {
            	WellInteger well = new WellInteger(index.row(), index.column());
                ValUtil.validateWell(this.rows,  this.columns, well);
                success = this.removeWells(well) ? success : false;
            } catch(Exception e) {
                System.err.println(e.getMessage());
     ...
     * does not exist.
     * @param    WellInteger    the well for replacement
     * @return                  true on successful well replacement
     */
    public boolean replaceWells(WellInteger well) {
     ...
     */
    public boolean replaceWells(WellSetInteger set) {
     ...
     * @return                              true on successful replacement of all wells
     */
    public boolean replaceWells(Collection<WellInteger> collection) {
     ...
     * @return                    true on successful replacement of all wells
     */
    public boolean replaceWells(WellInteger[] array) {
     ...
     */
    public boolean retainWells(WellInteger well) {
     ...
     */
    public boolean retainWells(WellSetInteger set) {
     ...
     * @return                              true on successful retention of all wells
     */
    public boolean retainWells(Collection<WellInteger> collection) {
     ...
     * @return                    true on successful retention of all wells
     */
    public boolean retainWells(WellInteger[] array) {
    ...
    public boolean retainWells(String wellList, String delimiter) {
        ...
        for(String well : split) {
            try {
            	WellInteger toRetain = new WellInteger(well);
    ...
    public boolean retainWells(WellIndex index) {
    	return this.retainWells(new WellInteger(index.row(), index.column()));
    }
     ...
     */
    public WellInteger getWells(WellInteger well) {
    	
    	Preconditions.checkNotNull(well, "Well cannot be null.");
    	
    	for(WellInteger result : this.data) {
    ...
    /**
     * Returns the well or null if no such well exists.
     * @param    WellSetInteger    the well
     * @return                     the well set
     */
    public WellSetInteger getWells(WellSetInteger set) {
        ...
        for(WellInteger well : set) {
        	...
        	WellInteger toAdd = this.getWells(new WellInteger(well.row(), well.column()));
    ...
    public WellInteger getWells(String well) {
        ...
        for(WellInteger toReturn : this.data) {
    ...
    public WellInteger getWells(WellIndex index) {
        ...
        for(WellInteger well : this.data) {
    ...
    public WellSetInteger getWells(WellList list) {
        ...
        for(WellIndex index : list) {
        	...
        	WellInteger well = this.getWells(index);
    ...
    public boolean contains(String wellList, String delimiter) {
        ...
        for(String well : split) {
            ...
            if(!this.contains(new WellInteger(well))) {
                return false;
            }
        }
    ...
    public boolean contains(String well) {
        return this.data.contains(new WellInteger(well));
    }
    ...
    public boolean contains(WellIndex index) {
    	...
    	return this.data.contains(new WellInteger(index.row(), index.column()));
    }
     ...
     */
    public <T extends Number> String printData(WellInteger well) {
     ...
     */
     public boolean equals(Object object) {
        ...
        PlateInteger plate = (PlateInteger) object;
     ...
     * @param    PlateInteger plate the plate for comparison
     * @return       this == plate --> 0
     *               this > plate --> 1
     *               this < plate --> -1
     */
    public int compareTo(PlateInteger plate) {
     ...
     *                           plate1 < plate2 --> -1
     */
    public int compare(PlateInteger plate1, PlateInteger plate2) throws ClassCastException {
